...
const MainUsers = () => {
    ...
    try {
      if (locationId) {
        ...
        setLocationName(response.data.name)
          ...
          try {
            // Fetch room details using the new endpoint
            const response = await api.get(`/locations/room-details/${roomId}`)
            setLocationName(response.data.name)
          } catch (e) {
              console.error("Error fetching room details", e);
              setLocationName("Room #" + roomId);
          }
      }
    } catch (error) {
  ...
  }, [locationId, roomId, role, fetchUsers, fetchLocationName, fetchLocations])
