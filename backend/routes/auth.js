...
router.get("/profile", authenticate(), async (req, res) => {
  try {
    const [result] = await db.query(
      `SELECT u.id, u.name, u.email, u.role, u.department, u.phone, u.status, u.unpk, u.ownpk, u.created_at, u.org_id, 
...
router.put("/profile", authenticate(), async (req, res) => {
  const { name, email, department, phone, status } = req.body;
  if (!name || !email) {
  ...
  try {
    await db.query(
      "UPDATE users SET name=?, email=?, department=?, phone=?, status=? WHERE id=?",
      [name, email, department || null, phone || null, status || 'Active', req.user.id] // Default status to Active if not provided/null? Or keep existing?
      // Actually, better to only update if provided, or assume frontend sends current values. 
      // The current frontend implementation sends everything in profileData. 
      // If status is undefined in req.body, it might overwrite with 'Active' if I use || 'Active'. 
      // Secure approach: only allow specific statuses.
    );
    // Let's refine the query to be safer or just stick to the plan which implies sending all data.
    // The previous code was: "UPDATE users SET name=?, email=?, department=? WHERE id=?"
    // I will stick to the standard update pattern used here.

    // However, wait. If the user is just updating their profile, should they be able to change their status?
    // The requirement says "add phone number and user status on leave or active using dropdown in profile information".
    // So yes, the user can change it.

    await db.query(
      "UPDATE users SET name=?, email=?, department=?, phone=?, status=? WHERE id=?",
      [name, email, department || null, phone || null, status || 'Active', req.user.id]
    );
